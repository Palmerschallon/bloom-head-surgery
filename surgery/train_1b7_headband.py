#!/usr/bin/env python3
"""
BLOOM-1b7 Head-Band Surgery: H9-H15 across all sick layers (L5-L22).

Stock BLOOM-1b7 has 140 sick heads total. 113 of those fall in the H9-H15 band.
This run reinitializes the H9-H15 BOS-sink heads across 18 layers simultaneously.

Architecture: 24 layers, 16 heads, 2048 hidden, 128 head dim (same as 560m skeleton).
Trainable: ~113 heads × ~1M params/head ≈ 118M params (~6.9% of 1.72B).

Corpus: curated Python + identity (polyglot) + structural anchor.
Start from: stock bigscience/bloom-1b7 (full 250K vocab, no surgery).
"""
...
# ═══════════════════════════════════════════
# SURGERY MAP — H9-H15 band across 18 layers
# ═══════════════════════════════════════════
#
# For each sick layer, we reinit H9-H15 heads that are BOS-sink/DEAD,
# and freeze H9-H15 heads that are already live.
# Heads H0-H8 are completely untouched (outside the band).
#
# From diagnostic: bigscience/bloom-1b7 stock weights
#
SURGERY = {
    5: {
        'reinit': [14, 15],                     # 2 BOS-sink
        'freeze': [9, 10, 11, 12, 13],          # 5 live in band
    },
    6: {
        'reinit': [9, 10, 11, 12, 13, 14, 15],  # 7 BOS-sink (all in band)
        'freeze': [],
    },
    7: {
        'reinit': [9, 10, 11, 12, 13, 14],      # 6 BOS-sink
        'freeze': [15],                           # 1 live in band
    },
    8: {
        'reinit': [11, 12, 13, 14, 15],          # 5 BOS-sink (H7 sick but outside band)
        'freeze': [9, 10],                        # 2 live in band
    },
    9: {
        'reinit': [9, 10, 11, 12, 13, 14, 15],  # 7 (6 BOS-sink + 1 DEAD H15)
        'freeze': [],
    },
    10: {
        'reinit': [9, 10, 11, 12, 13, 14, 15],  # 7 (6 BOS-sink + 1 DEAD H13)
        'freeze': [],
    },
    11: {
        'reinit': [10, 11, 12, 13, 14, 15],     # 6 (5 BOS-sink + 1 DEAD H11)
        'freeze': [9],                            # 1 live in band
    },
    12: {
        'reinit': [9, 10, 11, 12, 13, 14, 15],  # 7 BOS-sink
        'freeze': [],
    },
    13: {
        'reinit': [9, 10, 11, 12, 13, 14, 15],  # 7 BOS-sink (H2 sick but outside band)
        'freeze': [],
    },
    14: {
        'reinit': [9, 10, 11, 13, 14, 15],      # 6 BOS-sink
        'freeze': [12],                           # 1 live in band
    },
    15: {
        'reinit': [9, 11, 12, 14, 15],           # 5 BOS-sink
        'freeze': [10, 13],                       # 2 live in band
    },
    16: {
        'reinit': [10, 11, 12, 13, 14, 15],     # 6 BOS-sink
        'freeze': [9],                            # 1 live in band
    },
    17: {
        'reinit': [9, 10, 11, 12, 13, 14],      # 6 BOS-sink
    ...
    21: {
        'reinit': [9, 10, 11, 12, 13],          # 5 BOS-sink
        'freeze': [14, 15],                       # 2 live in band
    },
    22: {
        'reinit': [9, 10, 11, 13, 14],          # 5 BOS-sink
        'freeze': [12, 15],                       # 2 live in band
    },
}
...
log = logging.getLogger("1b7_headband")
...
def main():
    parser = argparse.ArgumentParser(description="BLOOM-1b7 Head-Band Surgery: H9-H15")
    ...
    parser.add_argument("--early_stop_patience", type=int, default=6,
                        help="Stop if PPL doesn't improve for N epochs")
    args = parser.parse_args()

    device = "cuda" if torch.cuda.is_available() else "cpu"

    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    run_dir = OUTPUT_BASE / f"headband_{ts}"
    ...
    log.info("=" * 60)
    log.info("BLOOM-1b7 HEAD-BAND SURGERY: H9-H15")
    log.info("=" * 60)
    ...
    log.info(f"  Band:        H9-H15 (systematic BOS-sink pattern)")
    ...
    trajectory = {
        'config': {
            'run': '1b7_headband',
            'description': 'BLOOM-1b7 H9-H15 band surgery across L5-L22 (113 heads)',
            'model': args.model,
            'epochs': args.epochs,
            'lr': args.lr,
    ...
    log.info(f"\n  Remaining sick heads outside H9-H15 band:")
    log.info(f"    L8-H7, L9-H3/H6/H8, L10-H3, L11-H3/H5/H6/H8,")
    log.info(f"    L12-H5/H8, L13-H2, L14-H4/H7/H8, L15-H3/H7/H8,")
    log.info(f"    L16-H3/H5/H8, L17-H3/H4/H6, L18-H2/H5/H8,")
    log.info(f"    L19-H3/H6/H8, L20-H7, L21-H3 = 27 heads for pass 2")
    log.info("\nDone.")
